...
> [!IMPORTANT]
> CDC 模式为高级功能，需要额外的服务器资源。详细配置请参考 [CDC 实时同步](../advanced/cdc-realtime.md)。
...
| [源平台配置](source-platform-config.md) | 配置数据查询参数和主键字段 |
| [目标平台配置](target-platform-config.md) | 配置数据写入规则和映射关系 |
| [监控告警](monitoring-alerts.md) | 配置运行监控和异常告警 |
| [CDC 实时同步](../advanced/cdc-realtime.md) | 了解 CDC 实时同步的高级配置 |
